...
for i = 1:n-1
    ...
    mask = coeff >= 0;
    coeff(coeff < 0)=0;
    A(m*2+1+2*(i-1),1:2*n) = (factorial(deg-1)./factorial(coeff)).*(time_stamps(1).^((1:2*n)-i-1)).*mask;
    A(m*2+2+2*(i-1),end-2*n+1:end) = (factorial(deg-1)./factorial(coeff)).*(time_stamps(end).^((1:2*n)-i-1)).*mask;
...
for i = 1:2*(n-1)
    ...
    for j = 1:m-1
        A((i-1)*(m-1)+j+end-2*(n-1),(j-1)*2*n+1:(j-1)*2*n+4*n)=...
            [(factorial(deg-1)./factorial(coeff)).*(time_stamps(j+1).^((1:2*n)-i-1)) -(factorial(deg-1)./factorial(coeff)).*(time_stamps(j+1).^((1:2*n)-i-1))].*[mask mask];
    end
end
