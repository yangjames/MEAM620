function [X,Z] = ekf2_nb(sensor, params)
...
if isempty(X_prev)
    ...
    t_prev_sen = 0;
    P = eye(9);
    Q = eye(6);%bsxfun(@times,eye(12),[ones(1,3)*10 ones(1,3) ones(1,3) ones(1,3)]);
    R = eye(9);
...
F = eye(9) + dt * params.A2_nb(phi,theta,psi,...
                            a_x,a_y,a_z,...
                            0,0,0,...
                            0,0,...
                            phi_dot,psi_dot);

bt = params.f2_nb(phi,theta,psi,...
    a_x,a_y,a_z,...
    0,0,0,...
    0,0,0,...
    v_x,v_y,v_z,...
...
%}
V = params.U2_nb(phi,theta,psi)*dt;
